<script lang="ts">
	...
	$: on_lights_class =
		device.currentValue === '1'
			? `${data.colors[type][400]} text-white border-white`
			: `bg-white text-lights-400`;
	$: on_media_class =
		device.currentValue === '1'
			? `${data.colors[type][400]} text-white border-white`
			: `bg-white text-media-400`;
	$: on_climate_class =
		device.currentValue === '1'
			? `${data.colors[type][400]} text-white border-white`
			: `bg-white text-climate-400`;
</script>
...
<section class="px-5 py-4 md:px-28 md:py-4">
	{#if $page.url.pathname.split('/')[1] === 'lights'}
		...
		<section class="pt-8">
			<button
				...
				class="{on_lights_class} border-2 font-bold rounded-2xl py-4 px-20"
			>
	...
	{:else if $page.url.pathname.split('/')[1] === 'media'}
		...
		<section class="pt-8">
			<button
				on:click={toggleDevice}
				class="{on_media_class} border-2 font-bold rounded-2xl py-4 px-20"
			>
				{#if device.currentValue == '0'}
					Turn On
				{:else}
	...
	{:else}
		...
		<section class="pt-8">
			<button
				on:click={toggleDevice}
				class="{on_climate_class} border-2 font-bold rounded-2xl py-4 px-20"
			>
				{#if device.currentValue == '0'}
					Turn On
				{:else}
